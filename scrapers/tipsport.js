...
export const scrapeTipsport = async () => {
	...
	try {
		...
		for (const index of menuIndexes) {
			...
			for (let i = 0; i < matches.length; i++) {
				...
				await page.evaluate(
					(selector, matchIndex) => {
						const element =
							document.querySelectorAll(selector)[matchIndex]
				...
				const betsGroups = await page.$$eval(BETS_SELECTOR, (elements) =>
					elements.map((groupEl, groupIndex) => {
						...
						const options = betButtonElsArray.map(
							(betButtonEl, index) => {
								const betName =
									betButtonEl.querySelector('span')?.textContent ||
									null
		...
		log(`...tipsport scraper error - ${err}`, false, 'error')
	}
